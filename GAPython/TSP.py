...
distances = generate_table(cities)

def fitness(order):
    n = len(order)
    return sum([distances[order[i]][order[i+1]] for i in range(n - 1)]) + distances[order[-1]][order[0]]
fitnes = []
for i in range(100000):
    genes = random.sample([i for i in range(0, 150)], 150)
    fitnes.append(fitness(genes))
